...
class Gren {
    constructor(props = {}) {
        ...
        this.options.addresses = utils.convertStringToArray(addresses);
        this.options.notifier = notifier;
    ...
    /**
     * Generate release notes and send it to an email address list
     *
     * @since  0.10.0
     * @public
     *
    ...
    /**
     * Send the release notes to an email address list
     *
     * @since  0.10.0
     * @public
     *
    ...
    _notify(blocks) {
        if (blocks.length > 0) {
            ...
            blocks.forEach(async block => {
                ...
                let html = converter.makeHtml(block.body);

                html = '<div><h2>Release changelog:</h2></div>'.concat(html);

                const mailOptions = {
                    from: this.options.notifier,
                    to: this.options.addresses,
                    subject: `${this.options.repo} - [${block.name}]`,
                    html: html
                };
